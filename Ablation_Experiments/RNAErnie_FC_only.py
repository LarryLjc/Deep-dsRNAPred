...
# =============================================================================
# 3. Core Model Architecture: Deep_dsRNAPred (Baseline FC Version)
# =============================================================================

class Deep_dsRNAPred(nn.Module):
    def __init__(self, max_seq_length, input_size=50, num_layers=3, num_dims=64, 
                 dropout_rate=0.2, num_classes=2):
        super(Deep_dsRNAPred, self).__init__()
        
        # Pure FC Network: Flatten Input
        # Input Dim = Seq_Len * Feature_Dim
        self.flatten_dim = max_seq_length * input_size
    ...
    def forward(self, x):
        """Forward: Input[B, L, C] -> Flatten -> FC Layers -> Output[B, 2]"""
        
        # 1. Flatten
        x = x.flatten(start_dim=1)  # [B, Length * Input_Size]
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    print(f"\n===== Fold {fold + 1}/{KFOLD} (Baseline: Pure FC) =====")
...
if __name__ == "__main__":
    ...
    print(f"Parameters: SeqLen={MAX_SEQ_LENGTH}, Feature_Dim={RNA_ERNIE_HIDDEN_DIM}")
