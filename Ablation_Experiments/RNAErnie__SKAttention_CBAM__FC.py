...
EPOCHS = 60
DROPOUT_RATE = 0.2
CBAM_LAYERS = 2  # Default CBAM layers
...
# =============================================================================
# 4. Core Model Architecture: Deep_dsRNAPred (NO BiLSTM)
# =============================================================================
...
class Deep_dsRNAPred(nn.Module):
    def __init__(self, max_seq_length, input_size=50, cnn_layers=3, cnn_dims=256, 
                 ...
                 num_classes=2, cbam_layers=CBAM_LAYERS): 
        ...
        # --- FC Layers (No LSTM) ---
        # Flatten Dim = Channels * Length
        self.feature_flatten_dim = self.cnn_dims * self.after_cnn_length
        
        self.fc_blocks = nn.ModuleList()
        in_features = self.feature_flatten_dim
    ...
    def forward(self, x):
        """Forward: Input[B, L, C] -> SK-CNN -> CBAM -> Flatten -> FC -> Output[B, 2]"""
        # Input to CNN format: [B, C, L, 1]
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print(f"5-Fold CV Summary (Model: Deep_dsRNAPred (No LSTM) | Feature: RNA-Ernie + Local PCA)")
    print(f"Parameters: SeqLen={MAX_SEQ_LENGTH}, Feature_Dim={RNA_ERNIE_HIDDEN_DIM}, CBAM={CBAM_LAYERS}")
    ...
    result_save_path = "Model_Performance_Deep_dsRNAPred_NoLSTM.xlsx"
