...
# --- Random Seed Setup (Critical for Reproducibility) ---
def setup_seed(seed):
...
LSTM_LAYERS = 2
SE_LAYERS = 1  # SEAttention layers (replaces CBAM_LAYERS)
...
# =============================================================================
# 3. SEAttention Module (Replaces CBAM)
# =============================================================================
...
# =============================================================================
# 5. Core Model Architecture: Deep_dsRNAPred
# =============================================================================
...
class Deep_dsRNAPred(nn.Module):
    def __init__(self, max_seq_length, input_size=50, cnn_layers=3, cnn_dims=256, 
        ...
        # --- SE Attention Blocks (Replaces CBAM) ---
        self.se_blocks = nn.ModuleList()
        for _ in range(se_layers):
            self.se_blocks.append(SEAttention(channel=cnn_dims, reduction=16))
    ...
    def forward(self, x):
        """Forward: Input[B, L, C] -> SK-CNN -> SE -> BiLSTM -> Flatten -> FC -> Output"""
        # [B, C, L, 1]
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    model = Deep_dsRNAPred(
        ...
        dropout_rate=0.2, num_classes=config.num_classes, 
        lstm_hidden_size=LSTM_HIDDEN_SIZE, lstm_layers=LSTM_LAYERS, se_layers=SE_LAYERS
    ).to(config.device)
    ...
    print(f"\n===== Fold {fold + 1}/{KFOLD} (SE Layers: {SE_LAYERS}) =====")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print(f"5-Fold CV Summary (Model: Deep_dsRNAPred (SK+SE+BiLSTM) | Feature: RNA-Ernie + Local PCA)")
    print(f"Parameters: SeqLen={MAX_SEQ_LENGTH}, LSTM_Dim={LSTM_HIDDEN_SIZE}, SE_Layers={SE_LAYERS}")
    ...
    result_save_path = "Model_Performance_Deep_dsRNAPred_SEAttention.xlsx"
