...
# Initialize Tokenizer & Model (Global for extraction only)
...
class Deep_dsRNAPred(nn.Module):
    def __init__(self, max_seq_length, input_size=50, cnn_layers=3, cnn_dims=256, 
        ...
        for _ in range(bam_layers):
            self.bam_blocks.append(BAMBlock(channel=cnn_dims, reduction=16, dia_val=2))
    ...
    def forward(self, x):
        """Forward: Input[B, L, C] -> SK-CNN -> BAM -> BiLSTM -> Flatten -> FC -> Output"""
        # [B, C, L, 1]
        ...
        # BAM
        for bam_block in self.bam_blocks:
            x = bam_block(x)
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    model = Deep_dsRNAPred(
        ...
        lstm_hidden_size=LSTM_HIDDEN_SIZE, lstm_layers=LSTM_LAYERS, bam_layers=BAM_LAYERS
    ).to(config.device)
    ...
    print(f"\n===== Fold {fold + 1}/{KFOLD} (BAM Layers: {BAM_LAYERS}) =====")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print(f"5-Fold CV Summary (Model: Deep_dsRNAPred (SK+BAM+BiLSTM) | Feature: RNA-Ernie + Local PCA)")
    print(f"Parameters: SeqLen={MAX_SEQ_LENGTH}, LSTM_Dim={LSTM_HIDDEN_SIZE}, BAM_Layers={BAM_LAYERS}")
    ...
    result_save_path = "Model_Performance_Deep_dsRNAPred_BAM.xlsx"
