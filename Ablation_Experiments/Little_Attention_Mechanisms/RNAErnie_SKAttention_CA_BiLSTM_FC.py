...
LSTM_LAYERS = 2
CA_LAYERS = 2  # Coordinate Attention layers
...
class Deep_dsRNAPred(nn.Module):
    def __init__(self, max_seq_length, input_size=50, cnn_layers=3, cnn_dims=256, 
        ...
        # --- Coordinate Attention (CA) Blocks ---
        self.ca_blocks = nn.ModuleList()
        for _ in range(ca_layers):
            self.ca_blocks.append(CoordAtt(inp=cnn_dims, oup=cnn_dims))
    ...
    def forward(self, x):
        """Forward: Input[B, L, C] -> SK-CNN -> CA -> BiLSTM -> Flatten -> FC -> Output"""
        # [B, C, L, 1]
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    model = Deep_dsRNAPred(
        ...
        lstm_hidden_size=LSTM_HIDDEN_SIZE, lstm_layers=LSTM_LAYERS, ca_layers=CA_LAYERS
    ).to(config.device)
    ...
    print(f"\n===== Fold {fold + 1}/{KFOLD} (CA Layers: {CA_LAYERS}) =====")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print(f"5-Fold CV Summary (Model: Deep_dsRNAPred (SK+CA+BiLSTM) | Feature: RNA-Ernie + Local PCA)")
    print(f"Parameters: SeqLen={MAX_SEQ_LENGTH}, LSTM_Dim={LSTM_HIDDEN_SIZE}, CA_Layers={CA_LAYERS}")
    ...
    result_save_path = "Model_Performance_Deep_dsRNAPred_CoordinateAttention.xlsx"
