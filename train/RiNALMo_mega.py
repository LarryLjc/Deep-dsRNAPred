...
# Path Configuration (请修改为你本地 RiNALMo 的实际路径)
...
def extract_RiNALMo_features(sequences, max_seq_length):
    hidden_states_list = []
    ...
    with torch.no_grad():
        for i in range(0, len(sequences), BATCH_SIZE):
            ...
            # 直接 padding 到 max_seq_length，不进行截断
            inputs = tokenizer(
                batch_seq, return_tensors="pt", padding="max_length",
                max_length=max_seq_length
            ).to(device)
            
            outputs = rinalmo_model(**inputs)
            # 提取最后一层隐藏状态作为序列特征
...
if __name__ == "__main__":
    ...
    X_cv_raw = extract_RiNALMo_features(cv_sequences, MAX_SEQ_LENGTH)
    y_cv = cv_df["label"].values

    print("Extracting Raw Features (Test)...")
    X_test_raw = extract_RiNALMo_features(test_sequences, MAX_SEQ_LENGTH)
    y_test = test_df["label"].values

    # --- CRITICAL STEP: DELETE MODEL TO PREVENT OOM KILL ---
    print("Feature extraction complete. Deleting RiNALMo model to free memory...")
    del rinalmo_model
    del tokenizer
    torch.cuda.empty_cache()
    gc.collect()
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_cv_raw)):
        ...
        pca = PCA(n_components=RINALMO_HIDDEN_DIM, random_state=SEED)
        ...
        # Reshape Back: (N, L, 50)
        X_train_fold = X_train_red.reshape(len(train_idx), seq_len, RINALMO_HIDDEN_DIM)
        X_val_fold = X_val_red.reshape(len(val_idx), seq_len, RINALMO_HIDDEN_DIM)
    ...
    print("\n" + "=" * 80)
    print(f"5-Fold CV Summary (Model: Deep_dsRNAPred | Feature: RiNALMo + Local PCA)")
    ...
    result_save_path = "Model_Performance_Deep_dsRNAPred_RiNALMo.xlsx"
