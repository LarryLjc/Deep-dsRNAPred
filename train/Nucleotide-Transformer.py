...
# --- 【修改】将原本嵌套的 PCA 分割成 fit 和 transform 两个独立函数 ---
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    # 将测试集从每一折内部剥离，只返回 Validation 指标
    return {
        'fold': fold + 1,
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_cv_raw)):
        ...
        # --- 保存 PCA 模型 ---
        pca_save_path = f'{SAVE_PKL_DIR}/nt_pca_fold_{fold + 1}.pkl'
        with open(pca_save_path, 'wb') as f:
            pickle.dump(ipca, f)
        print(f"PCA for Fold {fold + 1} saved to {pca_save_path}")
    ...
    # 【修改：加载每折保存的模型和PCA，对测试集打分并集成平均概率】
    print("\n" + "=" * 80)
    ...
    for fold in range(KFOLD):
        ...
        # 提取当前折拟合好的 PCA
        pca_load_path = f'{SAVE_PKL_DIR}/nt_pca_fold_{fold + 1}.pkl'
        ...
        del model, X_test_pca, ipca
        gc.collect()

    # 概率求平均作为集成的最终打分
    test_probs_avg = test_probs_sum / KFOLD
    test_preds = np.argmax(test_probs_avg, axis=1) # 等同于按照概率最高项判别类别(默认0.5阈值)
