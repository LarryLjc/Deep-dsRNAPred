...
# 全局模型初始化 (用于特征提取)
print(f"Loading Genos from {GENOS_PATH}...")
...
genos_model = AutoModel.from_pretrained(
    GENOS_PATH, 
...
def extract_Genos_features(sequences, max_seq_length):
    """
    Extract features using Genos.
    """
    ...
    for i in range(0, num_samples, BATCH_SIZE):
        ...
        # Mixtral 架构不接受 token_type_ids 参数，必须删除
...
def fit_pca_strict(X_train, n_components=50, batch_size=4096):
    """
    仅在 Train 上 fit PCA。
    """
...
def transform_pca_data(X_data, ipca, n_components=50, pca_chunk_size=100):
    """
    使用 fit 好的 PCA 对象进行 Transform 操作。
    """
...
if __name__ == "__main__":
    ...
    print("Extracting Raw Features for CV set using Genos...")
    X_cv_raw = extract_Genos_features(cv_sequences, MAX_SEQ_LENGTH)
    
    print("Extracting Raw Features for Test set using Genos...")
    X_test_raw = extract_Genos_features(test_sequences, MAX_SEQ_LENGTH)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_cv_raw)):
        ...
        # 2. 严格的内部 PCA (Fit on Train)
        ipca = fit_pca_strict(X_train_fold_raw, n_components=PCA_OUTPUT_DIM)
        # --- 【修改】保存当前折最好的 PCA 降维器 ---
        joblib.dump(ipca, f'pca_fold_{fold+1}.pkl')
        print(f"PCA for Fold {fold + 1} saved to pca_fold_{fold+1}.pkl")
        
        # 3. Transform Train 和 Val
        X_train_pca = transform_pca_data(X_train_fold_raw, ipca, n_components=PCA_OUTPUT_DIM)
        X_val_pca = transform_pca_data(X_val_fold_raw, ipca, n_components=PCA_OUTPUT_DIM)
        ...
        # 释放每折特有的内存
        del X_train_fold_raw, X_val_fold_raw, X_train_pca, X_val_pca
        gc.collect()

    # --- Step 4: Ensemble Testing ---
    # 【修改：加载每折保存的模型和PCA，对测试集独立打分然后集成】
    print("\n" + "=" * 80)
    print("Starting Ensemble Evaluation on Test Set...")
    
    # 初始化汇总矩阵，累加 5 折对每条测试集序列预测出的打分 (Softmax 概率)
    test_probs_sum = np.zeros((len(test_labels), NUM_CLASSES))

    for fold in range(KFOLD):
        print(f"Loading Model and PCA for Fold {fold + 1}...")
        
        # 加载 PCA 
        ipca = joblib.load(f'pca_fold_{fold+1}.pkl')
        ...
        model = Deep_dsRNAPred(
            ...
            num_layers=3, num_dims=64, dropout_rate=config.dropout,
    ...
    # 按照阈值 (默认为 0.5) 判定最终类别 
    # 即判断概率最大的那一项
    test_preds = np.argmax(test_probs_avg, axis=1) # 等同于跟 0.5 阈值对比
    ...
    print("\n" + "=" * 80)
    print(f"5-Fold CV Summary (Model: Deep_dsRNAPred, Feature: Genos+StrictPCA(50))")
