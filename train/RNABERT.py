...
# Path Configuration (请修改为你本地 RNABert 的实际路径)
...
print(f"Loading RNA-Bert from {RNA_BERT_PATH}...")
...
if __name__ == "__main__":
    ...
    X_cv_raw = extract_RNA_BERT_features(cv_sequences, MAX_SEQ_LENGTH)

    print("Extracting Raw Features (Test)...")
    X_test_raw = extract_RNA_BERT_features(test_sequences, MAX_SEQ_LENGTH)

    # --- CRITICAL STEP: DELETE MODEL TO PREVENT OOM KILL ---
    print("Feature extraction complete. Deleting RNA-Bert model to free memory...")
    del rna_bert_model
    del tokenizer
    torch.cuda.empty_cache()
    gc.collect()
    ...
    config = ModelConfig(
        ...
        input_size=RNA_BERT_HIDDEN_DIM, 
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_cv_raw)):
        ...
        pca = PCA(n_components=RNA_BERT_HIDDEN_DIM, random_state=SEED)
        ...
        # Reshape Back: (N, L, 50)
        X_train_fold = X_train_red.reshape(len(train_idx), seq_len, RNA_BERT_HIDDEN_DIM)
        X_val_fold = X_val_red.reshape(len(val_idx), seq_len, RNA_BERT_HIDDEN_DIM)
    ...
    # --- Step 4: Summary of Validation ---
    def get_avg_std(key):
        vals = [res[key] for res in all_fold_results]
        return np.mean(vals), np.std(vals)

    print("\n" + "=" * 80)
    print(f"5-Fold CV Summary (Model: Deep_dsRNAPred | Feature: RNA-Bert + Local PCA)")
    ...
    # ---------------------------------------------------------
    # 【修改】阶段 2：集成测试阶段 (Ensemble Testing)
    # ---------------------------------------------------------
    print("\n" + "=" * 80)
    print("Starting Ensemble Testing Phase (Soft Voting)...")
    
    # 初始化全零矩阵用于累加 5 折产生的测试集预测概率
    ...
    for fold in range(KFOLD):
        ...
        # 2. 对 Test 进行 PCA 降维，并重塑
        X_test_red = pca.transform(X_test_flat)
        X_test_fold = X_test_red.reshape(n_test, seq_len, RNA_BERT_HIDDEN_DIM)
        test_loader = build_dataloader(test_sequences, test_labels, X_test_fold, batch_size=BATCH_SIZE, shuffle=False)
        ...
        # 4. 获取预测概率并累加
        fold_probs = predict_test_probs(test_loader, model, config.device)
        ensemble_test_probs += fold_probs
        
        # 5. 清理内存
        del pca, X_test_red, X_test_fold, test_loader, model
        torch.cuda.empty_cache()
        gc.collect()

    # 概率求平均
    ensemble_test_probs /= KFOLD
    # 根据平均后的概率取最大值对应索引作为最终分类结果 (等同于 0.5 阈值)
    ...
    result_save_path = "Model_Performance_Deep_dsRNAPred_ValOnly.xlsx"
