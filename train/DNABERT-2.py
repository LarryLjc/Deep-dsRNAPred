...
import warnings
import gc
import joblib  # 新增：用于保存和加载 PCA 模型
from collections import OrderedDict
from torch.utils.data import Dataset, DataLoader
from torch.optim import lr_scheduler
from torch.nn import init

# Transformers & DNABERT-2
...
def extract_DNABERT2_features(sequences, max_seq_length):
...
def fit_transform_pca_strict(X_train, X_val, n_components=50, batch_size=4096):
    """
    仅在训练集上 Fit PCA，并 transform 训练集和验证集。返回降维后的数据和 PCA 模型
    """
    ...
    def transform_data(X_data, pca_model):
        ...
        for i in range(0, N, pca_chunk_size):
            ...
            chunk_flat = chunk.reshape(-1, C)
            chunk_reduced = pca_model.transform(chunk_flat)
    ...
    X_train_pca = transform_data(X_train, ipca)
    X_val_pca = transform_data(X_val, ipca)
    
    return X_train_pca, X_val_pca, ipca

def transform_pca_inference(X_data, ipca_model, n_components=50):
    """
    用于独立阶段对 Test 数据进行降维推理
    """
    N, L, C = X_data.shape
    pca_chunk_size = 100
    X_reduced_list = []
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    print(f"Fold {fold + 1} Best Val Result | "
          f"Val Sn: {best_val_metrics['Sn']:.1%} | Val Sp: {best_val_metrics['Sp']:.1%} | "
...
if __name__ == "__main__":
    ...
    print("Extracting Raw Features for CV set...")
    X_cv_raw = extract_DNABERT2_features(cv_sequences, MAX_SEQ_LENGTH)
    
    print("Extracting Raw Features for Test set...")
    X_test_raw = extract_DNABERT2_features(test_sequences, MAX_SEQ_LENGTH)
    
    gc.collect()

    # --- Step 3: 5-Fold CV (With strict PCA saving) ---
    config = ModelConfig(
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_cv_raw)):
        ...
        # 仅对 Train/Val 降维，并获取当前折的 PCA 模型
        X_train_pca, X_val_pca, ipca_model = fit_transform_pca_strict(
            X_train_fold_raw, X_val_fold_raw, 
            n_components=PCA_OUTPUT_DIM
        )
        
        # 保存 PCA 模型供测试阶段集成使用
        pca_save_path = f"{SAVE_PCA_PREFIX}{fold + 1}.pkl"
        joblib.dump(ipca_model, pca_save_path)
        ...
        del X_train_fold_raw, X_val_fold_raw, X_train_pca, X_val_pca, ipca_model
        gc.collect()
    
    # --- Step 4: Ensemble Testing (集成测试阶段) ---
    print("\n" + "=" * 80)
    print("Starting Ensemble Testing (Soft Voting over 5 Folds)...")
    
    # 初始化累加预测概率矩阵
    ensemble_probs = np.zeros((len(test_labels), NUM_CLASSES), dtype=np.float32)
    
    for fold in range(KFOLD):
        print(f"Loading Fold {fold + 1} PCA and Model for evaluation...")
        
        # 1. 加载当前折对应的 PCA 并转换测试集
        ...
        model.load_state_dict(torch.load(f"{SAVE_MODEL_PREFIX}{fold + 1}_best.pth", map_location=config.device))
        model.eval()
    ...
    # 利用阈值决定最终预测（argmax等价于以0.5为阈值分类。若需调整阈值，可使用 `final_preds = (ensemble_probs[:, 1] >= threshold).astype(int)`）
    final_preds = np.argmax(ensemble_probs, axis=1)
    
    # 计算集成后的综合指标
    ensemble_metrics = calculate_metrics(test_labels, final_preds, ensemble_probs)
    
    # --- Step 5: Summary & Saving ---
    ...
    print("\n" + "=" * 80)
    print(f"Cross Validation & Ensemble Test Summary")
    ...
    print(f"Sn:  {ensemble_metrics['Sn']: .3f}")
    print(f"Sp:  {ensemble_metrics['Sp']: .3f}")
    print(f"ACC: {ensemble_metrics['ACC']:.3f}")
    print(f"MCC: {ensemble_metrics['MCC']:.3f}")
    print(f"F1:  {ensemble_metrics['F1']: .3f}")
    print(f"AUC: {ensemble_metrics['AUC']:.3f}")
    print("=" * 80)

    # 保存各折验证集性能以及最后的 Test 综合结果
    result_df = pd.DataFrame(all_fold_results)
    
    # 将集成的 Test 结果作为最后一行追加写入，方便在 Excel 内查看对比
    ensemble_row = {
        'fold': 'Ensemble_Test',
        'best_val_Sn': np.nan, 'best_val_Sp': np.nan, 'best_val_ACC': np.nan, 
        'best_val_MCC': np.nan, 'best_val_AUC': np.nan, 'best_val_F1': np.nan,
        'test_Sn': ensemble_metrics['Sn'], 'test_Sp': ensemble_metrics['Sp'],
        'test_ACC': ensemble_metrics['ACC'], 'test_MCC': ensemble_metrics['MCC'],
        'test_AUC': ensemble_metrics['AUC'], 'test_F1': ensemble_metrics['F1'],
        'test_FPR': ensemble_metrics['FPR'], 'test_TPR': ensemble_metrics['TPR']
    }
