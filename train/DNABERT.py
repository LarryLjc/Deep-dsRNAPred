...
# Path Configuration (请修改为你本地 DNABert 的实际路径)
...
print(f"Loading DNA-Bert from {DNA_BERT_PATH}...")
...
def resize_pos_embeddings(model, tokenizer, new_max_length):
    """运用一维线性插值动态拉伸 DNABert 的 Position Embeddings"""
    config = model.config
    ...
    print(f"\n[Info] Input length {new_max_length} > model limit {current_max_pos}.")
    print(f"Interpolating position embeddings to support {new_max_length} tokens...")
    
    # 动态定位 DNABert/BERT 的 embedding 层
    ...
    # 转换形状以适应插值函数: [1, Hidden_Dim, Seq_Length]
    ...
    # 替换原本的 position_embeddings
    embed_layer.position_embeddings = new_pos_layer
    
    # 如果存在 position_ids buffer，同步更新它
...
def extract_DNA_BERT_features(sequences, max_seq_length):
    # --- 【关键修改】在提取特征前，先调用线性插值重置模型最大长度 ---
    resize_pos_embeddings(dna_bert_model, tokenizer, max_seq_length)
    ...
    with torch.no_grad():
        for i in range(0, len(sequences), BATCH_SIZE):
            ...
            # 使用 output_hidden_states=True 强制 AutoModelForMaskedLM 输出特征层
            outputs = dna_bert_model(**inputs, output_hidden_states=True)
            # 提取最后一层隐藏状态作为序列特征
            batch_hidden = outputs.hidden_states[-1]
            
            # 安全检查：以防万一 tokenizer 补充长度不足
...
if __name__ == "__main__":
    ...
    # Determine True Max Seq Length
    MAX_SEQ_LENGTH = max([len(seq) for seq in cv_sequences + test_sequences])
    # --- 【关键修改】移除了硬截断逻辑，允许模型使用完整长度 ---
    print(f"\nMax Sequence Length: {MAX_SEQ_LENGTH}\n")

    # --- Step 2: Feature Extraction (Raw) ---
    print("Extracting Raw Features (Train)...")
    X_cv_raw = extract_DNA_BERT_features(cv_sequences, MAX_SEQ_LENGTH)
    y_cv = cv_df["label"].values

    print("Extracting Raw Features (Test)...")
    X_test_raw = extract_DNA_BERT_features(test_sequences, MAX_SEQ_LENGTH)
    y_test = test_df["label"].values

    # --- CRITICAL STEP: DELETE MODEL TO PREVENT OOM KILL ---
    print("Feature extraction complete. Deleting DNA-Bert model to free memory...")
    del dna_bert_model
    del tokenizer
    torch.cuda.empty_cache()
    gc.collect()
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_cv_raw)):
        ...
        pca = PCA(n_components=DNA_BERT_HIDDEN_DIM, random_state=SEED)
        ...
        # Reshape Back: (N, L, 50)
        X_train_fold = X_train_red.reshape(len(train_idx), seq_len, DNA_BERT_HIDDEN_DIM)
        X_val_fold = X_val_red.reshape(len(val_idx), seq_len, DNA_BERT_HIDDEN_DIM)
    ...
    print("\n" + "=" * 80)
    print(f"5-Fold CV Summary (Model: Deep_dsRNAPred | Feature: DNA-Bert + Local PCA)")
    ...
    result_save_path = "Model_Performance_Deep_dsRNAPred_DNABert.xlsx"
