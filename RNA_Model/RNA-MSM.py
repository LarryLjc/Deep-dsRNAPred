...
if __name__ == "__main__":
    ...
    X_cv_raw = extract_RNA_MSM_features(cv_sequences, MAX_SEQ_LENGTH)
    y_cv = cv_df["label"].values

    print("Extracting Raw Features (Test)...")
    X_test_raw = extract_RNA_MSM_features(test_sequences, MAX_SEQ_LENGTH)
    y_test = test_df["label"].values

    # --- Step 3: Cross Validation with Strict PCA ---
    config = ModelConfig(
        max_time_steps=MAX_SEQ_LENGTH,
        input_size=RNA_MSM_HIDDEN_DIM, 
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_cv_raw)):
        ...
        # Reshape Back: (N, L, 50)
        X_train_fold = X_train_red.reshape(len(train_idx), seq_len, RNA_MSM_HIDDEN_DIM)
        X_val_fold = X_val_red.reshape(len(val_idx), seq_len, RNA_MSM_HIDDEN_DIM)
        X_test_fold = X_test_red.reshape(n_test, seq_len, RNA_MSM_HIDDEN_DIM)
        ...
        # --- CRITICAL: Cleanup Memory after Fold ---
        # Explicitly delete the heavy variables specific to this fold
        ...
        del train_seqs_fold, val_seqs_fold, train_labels_fold, val_labels_fold
        del pca
    ...
    print("\n" + "=" * 80)
    print(f"5-Fold CV Summary (Model: Deep-dsRNAPred | Feature: RNA-MSM + Local PCA)")
