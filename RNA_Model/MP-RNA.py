...
# 维度与超参数配置
MP_RNA_HIDDEN_DIM = 50  # PCA降维目标维度
KFOLD = 5
NUM_CLASSES = 2
BATCH_SIZE = 64
EPOCHS = 40
...
def extract_MP_RNA_features(sequences, max_seq_length):
    ...
    with torch.no_grad():
        for i in range(0, len(sequences), BATCH_SIZE):
            ...
            # 立即转移到 CPU 并转为 float16 以节省内存 (如果后续 PCA 支持，或者保持 float32)
            # 这里保持 float32 以确保 PCA 精度，但立即 .cpu()
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_cv_raw)):
        ...
        # 1. 划分 Raw Data
        # [Memory Note]: Creating views or copies.
        ...
        # 2. PCA Fitting (仅在当前折的 Train 上 fit)
        # Reshape for PCA: [N, L, C] -> [N*L, C]
        ...
        # 4. Transform Val (使用 Train 的 PCA)
        ...
        # 5. Transform Test (使用当前折 Train 的 PCA)
        ...
        # [修复核心]: 立即删除 PCA 过程中的大变量
        del train_flat, train_red, val_flat, val_red, test_flat, test_red
        del train_features_raw, val_features_raw  # 删除当前折的 raw 副本
        gc.collect()
        ...
        # [修复核心]: 折叠结束后，彻底清理本折的所有变量，防止累积到下一折
    ...
    print("\n" + "=" * 80)
    print(f"5-Fold CV Summary (Model: Deep_dsRNAPred, Feature: MP-RNA+StrictPCA(50))")
