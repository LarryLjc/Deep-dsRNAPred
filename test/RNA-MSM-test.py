...
# 这里的长度必须和你之前跑训练代码时，终端打印出来的 "Auto-detected Max Sequence Length" 完全一致！
TRAINED_MAX_SEQ_LENGTH = 615  # <--- 请务必修改为真实的训练长度

# 模型和PCA文件的前缀 (需要和训练脚本保存的路径在同级目录，或修改为绝对路径)
...
if __name__ == "__main__":
    print(f"\n{'='*50}\nStarting Independent Ensemble Inference\n{'='*50}")
    ...
    print("\nExtracting RNA-MSM Embeddings...")
    X_test_raw = extract_RNA_MSM_features(test_sequences, TRAINED_MAX_SEQ_LENGTH)
    ...
    for fold in range(KFOLD):
        ...
        model = Deep_dsRNAPred(
            max_seq_length=TRAINED_MAX_SEQ_LENGTH, input_size=RNA_MSM_HIDDEN_DIM,
    ...
    # Calculate $P_{avg} = \frac{1}{K} \sum P_i$
