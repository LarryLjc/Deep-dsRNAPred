...
# =============================================================================
# 2. ORF FEATURE EXTRACTION LOGIC
# =============================================================================
...
class ORF_count():
    ...
    def len_cov(self, seq):
        (mRNA_size, CDS_size, CDS_integrity) = self.extract_feature_from_seq(seq=seq, stt=self.start_codons, stp=self.stop_codons)
        self.Coverage = float(CDS_size) / mRNA_size if mRNA_size > 0 else 0.0
        return (CDS_size, self.Coverage, CDS_integrity)
    ...
    def get_orf_frame_score(self, seq):
        ...
        l1 = len(ORF_length_in_frame1)
        l2 = len(ORF_length_in_frame2)
        l3 = len(ORF_length_in_frame3)
        ORF_len = [l1, l2, l3]
        
        ORF_frame = ((ORF_len[0] - ORF_len[1]) ** 2 + (ORF_len[0] - ORF_len[2]) ** 2 + (ORF_len[1] - ORF_len[2]) ** 2) / 2
        return ORF_frame

def extract_ORF_features(sequences, max_seq_length):
    ocr = ORF_count()
    features_list = []
    print(f"Extracting ORF features (Total: {len(sequences)})...")
    
    for idx, seq in enumerate(sequences):
        cds_len, cov, integrity = ocr.len_cov(seq)
        f_score = ocr.get_orf_frame_score(seq)
        
        # Feature Vector: [4]
        feat_vec = np.array([cds_len, cov, integrity, f_score], dtype=np.float32)
        
        # Broadcast to [Max_Seq_Length, 4] for model input compatibility
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    for epoch in range(epochs):
        ...
        if val_metrics['ACC'] > best_val_acc:
            ...
            torch.save(model.state_dict(), f"{config.save_model_prefix}{fold + 1}_best_ORF.pth")
    ...
    # Test Evaluation
    model.load_state_dict(torch.load(f"{config.save_model_prefix}{fold + 1}_best_ORF.pth", map_location=config.device))
...
if __name__ == "__main__":
    ...
    # 2. Extract Features
    X_cv = extract_ORF_features(cv_sequences, MAX_SEQ_LENGTH)
    y_cv = np.array(cv_labels)
    X_test = extract_ORF_features(test_sequences, MAX_SEQ_LENGTH)
    ...
    # 4. Summary & Save
    print("\n" + "=" * 100)
    print(f"5-Fold CV Summary (Model: Deep-dsRNAPred | Feature: ORF)")
    ...
    pd.DataFrame(all_fold_results).to_excel("Model_Performance_ORF.xlsx", index=False)
    print(f"Results saved to Model_Performance_ORF.xlsx")