...
print(f"K-mer size: {K_MER}, Feature Dimension: {FEATURE_DIM}")

# =============================================================================
# 2. K-MER FEATURE EXTRACTION LOGIC
# =============================================================================

class BasicCounter:
    """Generates overlapping kmer counts"""

    def __init__(self, seqs=None, k=6, mean=False, std=False, silent=False):
        self.seqs = seqs
        self.k = k
        self.mean = mean
        self.std = std
    ...
    def occurrences(self, row, seq):
        ...
        # Normalize by length (per kilobase)
    ...
    def get_counts(self):
        ...
        iterator = tqdm(self.seqs, desc=f'{self.k}-mer Counting', disable=self.silent)
        
        for i, seq in enumerate(iterator):
            self.counts[i] = self.occurrences(self.counts[i], seq.upper())
            
        if self.mean is not False: self.center()
        if self.std is not False: self.standardize()

        return self.counts

def extract_kmer_features_compact(sequences, k=6):
    """
    Extracts global K-mer statistics.
    Returns: [N, 4^k]
    """
    counter = BasicCounter(seqs=sequences, k=k)
    print(f"Starting {k}-mer feature extraction (Total: {len(sequences)})...")
    
    counts_matrix = counter.get_counts() 
    
    print("K-mer feature extraction complete (Compact Mode).")
    return counts_matrix.astype(np.float32)
...
# =============================================================================
# 5. DATA PROCESSING & UTILS (Memory Optimized)
# =============================================================================

class RNADataset(Dataset):
    def __init__(self, sequences, labels, features=None, max_len=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.features is not None:
            # 1. Get Compact Feature Vector [4096]
            feat_vec = torch.tensor(self.features[idx], dtype=torch.float32)
            
            # 2. Expand on the fly to [Max_Len, 4096]
            feature_matrix = feat_vec.unsqueeze(0).expand(self.max_len, -1)
            
            return feature_matrix, label
            
        return self.sequences[idx], label

def build_dataloader(sequences, labels, features=None, max_len=None, batch_size=32, shuffle=True):
    dataset = RNADataset(sequences, labels, features, max_len=max_len)
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    print(f"\n===== Fold {fold + 1}/{KFOLD} Training (Feature: {K_MER}-mer) =====")
    ...
    for epoch in range(epochs):
        ...
        if val_metrics['ACC'] > best_val_acc:
            ...
            torch.save(model.state_dict(), f"{config.save_model_prefix}{fold + 1}_best_Kmer.pth")
    ...
    # Test Evaluation
    model.load_state_dict(torch.load(f"{config.save_model_prefix}{fold + 1}_best_Kmer.pth", map_location=config.device))
...
if __name__ == "__main__":
    ...
    # 2. Extract Features (Compact Mode)
    print(f"\n>>> Extracting Train {K_MER}-mer Features (Dim={FEATURE_DIM})...")
    X_cv = extract_kmer_features_compact(cv_sequences, k=K_MER)
    y_cv = np.array(cv_labels)
    X_test = extract_kmer_features_compact(test_sequences, k=K_MER)
    y_test = np.array(test_labels)

    print(f"Feature Shape (Compact) | CV: {X_cv.shape} | Test: {X_test.shape}")
    
    # 3. K-Fold Cross Validation
    test_loader = build_dataloader(test_sequences, test_labels, X_test, 
    ...
    # 4. Summary & Save
    print("\n" + "=" * 100)
    print(f"5-Fold CV Summary (Model: Deep-dsRNAPred | Feature: {K_MER}-mer | Dim: {FEATURE_DIM})")
    ...
    pd.DataFrame(all_fold_results).to_excel("Model_Performance_Kmer.xlsx", index=False)
    print(f"Results saved to Model_Performance_Kmer.xlsx")