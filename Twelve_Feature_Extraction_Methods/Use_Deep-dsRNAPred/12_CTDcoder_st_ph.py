...
SAVE_MODEL_PREFIX = 'stage1model_fold_ctdp_'
...
def extract_ctd_features(sequences, max_seq_length):
    ...
    print(f"Extracting CTD feature extraction (Total: {len(sequences)})...")
    
    for idx, seq in enumerate(sequences):
        seq = seq.upper().replace('U', 'T')
        
        # 1. Extract 91 scalar features
        f_vec = ctd_coder.get_features(seq)
        
        # 2. Convert to numpy array [91]
        feat_vec = np.array(f_vec, dtype=np.float32)
        
        # 3. Broadcast to sequence length [Max_Seq_Length, 91]
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    print(f"\n===== Fold {fold + 1}/{KFOLD} Training (Feature: CTD_Physio) =====")
    ...
    for epoch in range(epochs):
        ...
        if val_metrics['ACC'] > best_val_acc:
            ...
            torch.save(model.state_dict(), f"{config.save_model_prefix}{fold + 1}_best_CTDP.pth")
    ...
    # Test Evaluation
    model.load_state_dict(torch.load(f"{config.save_model_prefix}{fold + 1}_best_CTDP.pth", map_location=config.device))
...
if __name__ == "__main__":
    ...
    # 4. Summary & Save
    print("\n" + "=" * 100)
    print(f"5-Fold CV Summary (Model: Deep-dsRNAPred | Feature: CTD_Physio | Dim: {FEATURE_DIM})")
    ...
    pd.DataFrame(all_fold_results).to_excel("Model_Performance_CTD_Physio.xlsx", index=False)
    print(f"Results saved to Model_Performance_CTD_Physio.xlsx")