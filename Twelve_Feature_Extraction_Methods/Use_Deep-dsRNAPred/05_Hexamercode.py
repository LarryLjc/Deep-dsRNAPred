...
SAVE_MODEL_PREFIX = 'stage1model_fold_hex_'

# Hyperparameters
FEATURE_DIM = 1  # Hexamer Score is a single scalar
KFOLD = 5
NUM_CLASSES = 2
BATCH_SIZE = 256
EPOCHS = 60
...
# =============================================================================
# 2. HEXAMER FEATURE EXTRACTION LOGIC
# =============================================================================
...
def kmer_freq_list(seq_list, word_size, step_size=1, frame=0, min_count=0):
    seq_num = 0
    ret_dict = {}
...
class Hexamercoder():
    def __init__(self, word_size=6, step_size=3, coding_seqs=None, noncoding_seqs=None):
        self.coding_seqs = coding_seqs
        self.noncoding_seqs = noncoding_seqs
        self.word_size = word_size
        self.step_size = step_size
        self.coding = {}
        self.noncoding = {}
        
        if self.coding_seqs is not None and self.noncoding_seqs is not None:
    ...
    def coding_nocoding_potential(self):
        cod = kmer_freq_list(seq_list=self.coding_seqs, word_size=6, step_size=3, frame=0)
        noncod = kmer_freq_list(seq_list=self.noncoding_seqs, word_size=6, step_size=1, frame=0)

        cod_sum = 0.000001 + sum(cod.values())
        noncod_sum = 0.000001 + sum(noncod.values())
        ...
        for kmer in cod:
            if 'N' in kmer: continue
            coding[kmer] = float(cod[kmer] / cod_sum)
            noncoding[kmer] = float(noncod[kmer] / noncod_sum)
            
        return coding, noncoding

    def get_score(self, seq):
        seq = seq.upper()
        return self.kmer_ratio(seq, self.coding, self.noncoding)

def extract_hexamer_features(sequences, max_seq_length, hex_coder):
    features_list = []
    print(f"Extracting Hexamer features (Total: {len(sequences)})...")
    
    for idx, seq in enumerate(sequences):
        seq = seq.upper().replace('U', 'T')
        
        # 1. Extract scalar feature (Hexamer Score)
        score = hex_coder.get_score(seq)
        
        # 2. Convert to numpy array [1]
        feat_vec = np.array([score], dtype=np.float32)
        
        # 3. Broadcast to sequence length [Max_Seq_Length, 1]
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    print(f"\n===== Fold {fold + 1}/{KFOLD} Training (Feature: Hexamer) =====")
    ...
    for epoch in range(epochs):
        ...
        if val_metrics['ACC'] > best_val_acc:
            ...
            torch.save(model.state_dict(), f"{config.save_model_prefix}{fold + 1}_best_Hex.pth")
    ...
    # Test Evaluation
    model.load_state_dict(torch.load(f"{config.save_model_prefix}{fold + 1}_best_Hex.pth", map_location=config.device))
...
if __name__ == "__main__":
    ...
    # 2. Extract Features (Hexamer)
    print("\n>>> Building Hexamer Tables from CV Data...")
    
    # Coding (label=1) vs Noncoding (label=0) for potential calculation
    cv_coding_seqs = [seq for seq, label in zip(cv_sequences, cv_labels) if label == 1]
    cv_noncoding_seqs = [seq for seq, label in zip(cv_sequences, cv_labels) if label == 0]
    
    # Initialize coder with training data to build log-likelihood ratios
    hex_coder = Hexamercoder(word_size=6, step_size=3, coding_seqs=cv_coding_seqs, noncoding_seqs=cv_noncoding_seqs)
    
    print("\n>>> Extracting Train Hexamer Features (Dim=1)...")
    X_cv = extract_hexamer_features(cv_sequences, MAX_SEQ_LENGTH, hex_coder)
    y_cv = np.array(cv_labels)

    print("\n>>> Extracting Test Hexamer Features (Dim=1)...")
    X_test = extract_hexamer_features(test_sequences, MAX_SEQ_LENGTH, hex_coder)
    ...
    # 4. Summary & Save
    print("\n" + "=" * 100)
    print(f"5-Fold CV Summary (Model: Deep-dsRNAPred | Feature: Hexamer | Dim: {FEATURE_DIM})")
    ...
    pd.DataFrame(all_fold_results).to_excel("Model_Performance_Hexamer.xlsx", index=False)
    print(f"Results saved to Model_Performance_Hexamer.xlsx")