...
SAVE_MODEL_PREFIX = 'stage1model_fold_sparse_'

# Hyperparameters
FEATURE_DIM = 3  # Sparse Encoding maps each base to a vector of size 3
KFOLD = 5
NUM_CLASSES = 2
BATCH_SIZE = 256
EPOCHS = 60
...
# =============================================================================
# 2. SPARSE ENCODING FEATURE EXTRACTION LOGIC
# =============================================================================

class SparseCoder:
    """
    Generates Sparse Encoding for DNA/RNA sequences based on provided dictionary.
    """
    def __init__(self, max_len):
        self.max_len = max_len
        # Mapping from provided code
        self.group_dict = {
            'A': [1, 1, 1],
            'T': [0, 0, 1],
            'C': [1, 0, 0],
            'G': [0, 1, 0],
            # Add basic handling for lower case and RNA 'U' mapping to 'T'
            'a': [1, 1, 1],
            't': [0, 0, 1],
            'c': [1, 0, 0],
            'g': [0, 1, 0],
    ...
    def encode(self, seq):
        """
        Encodes a single sequence string into a [Max_Len, 3] matrix.
        Implements the logic: 
        - If len >= N, take [0:N]
        - If len < N, fill [0:len] and leave rest as zeros
        """
        # Initialize zero matrix [N, 3]
        encoding_array = np.zeros((self.max_len, 3), dtype=np.float32)
        
        # Calculate effective length (truncate if necessary)
        seq_len = min(len(seq), self.max_len)
        
        # Fill array
        for i in range(seq_len):
            base = seq[i]
            if base in self.group_dict:
                encoding_array[i, :] = self.group_dict[base]
            # Else remains [0, 0, 0] (e.g. for 'N')
            
        return encoding_array

def extract_sparse_features(sequences, max_seq_length):
    """
    Extracts Sparse Encoding features (3 dim) and handles padding/truncation.
    Output Shape: [Num_Seqs, Max_Seq_Length, 3]
    """
    coder = SparseCoder(max_len=max_seq_length)
    features_list = []
    
    print(f"Starting Sparse Encoding feature extraction (Total: {len(sequences)})...")
    
    for idx, seq in enumerate(sequences):
        if idx % 1000 == 0 and idx > 0:
            print(f"  Processed {idx} sequences...")
            
        # Clean sequence (handle mixed case/RNA)
        seq = seq.upper().replace('U', 'T')
        
        # 1. Encode to [Max_Len, 3] matrix
        sparse_mat = coder.encode(seq)
        
        features_list.append(sparse_mat)
        
    print("Sparse Encoding feature extraction complete.")
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    print(f"\n===== Fold {fold + 1}/{KFOLD} Training (Feature: SparseEncoding) =====")
    ...
    for epoch in range(epochs):
        ...
        if val_metrics['ACC'] > best_val_acc:
            ...
            torch.save(model.state_dict(), f"{config.save_model_prefix}{fold + 1}_best_Sparse.pth")
    ...
    # Test Evaluation
    model.load_state_dict(torch.load(f"{config.save_model_prefix}{fold + 1}_best_Sparse.pth", map_location=config.device))
...
if __name__ == "__main__":
    ...
    assert "label" in cv_df.columns and "sequence" in cv_df.columns, "Excel columns missing (expected 'label' and 'sequence')"
    assert "label" in test_df.columns and "sequence" in test_df.columns, "Excel columns missing (expected 'label' and 'sequence')"
    ...
    # 2. Extract Features (Sparse Encoding)
    print("\n>>> Extracting Train Sparse Encoding Features (Dim=3)...")
    X_cv = extract_sparse_features(cv_sequences, MAX_SEQ_LENGTH)
    y_cv = np.array(cv_labels)

    print("\n>>> Extracting Test Sparse Encoding Features (Dim=3)...")
    X_test = extract_sparse_features(test_sequences, MAX_SEQ_LENGTH)
    ...
    # 4. Summary & Save
    print("\n" + "=" * 100)
    print(f"5-Fold CV Summary (Model: Deep-dsRNAPred | Feature: SparseEncoding | Dim: {FEATURE_DIM})")
    ...
    pd.DataFrame(all_fold_results).to_excel("Model_Performance_Sparse.xlsx", index=False)
    print(f"Results saved to Model_Performance_Sparse.xlsx")