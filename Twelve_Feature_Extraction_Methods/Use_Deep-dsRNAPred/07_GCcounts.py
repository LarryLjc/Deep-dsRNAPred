...
SAVE_MODEL_PREFIX = 'stage1model_fold_gc_'

# Hyperparameters
FEATURE_DIM = 7  # GC Features
KFOLD = 5
NUM_CLASSES = 2
BATCH_SIZE = 256
EPOCHS = 60
...
class GCconder():
    ...
    def GC1(self, mRNA):
        if len(mRNA) < 3: numGC = 0
        else: numGC = mRNA[0::3].count("C") + mRNA[0::3].count("G")
        return numGC * 1.0 / len(mRNA) * 3

    def GC2(self, mRNA):
        if len(mRNA) < 3: numGC = 0
        else: numGC = mRNA[1::3].count("C") + mRNA[1::3].count("G")
        return numGC * 1.0 / len(mRNA) * 3

    def GC3(self, mRNA):
        if len(mRNA) < 3: numGC = 0
        else: numGC = mRNA[2::3].count("C") + mRNA[2::3].count("G")
        return numGC * 1.0 / len(mRNA) * 3
    ...
    def gc1_frame_score(self, seq):
        ...
        return ((GC1_all[0] - GC1_all[1]) ** 2 + (GC1_all[0] - GC1_all[2]) ** 2 + (GC1_all[1] - GC1_all[2]) ** 2) / 2
    ...
    def gc2_frame_score(self, seq):
        ...
        return ((GC2_all[0] - GC2_all[1]) ** 2 + (GC2_all[0] - GC2_all[2]) ** 2 + (GC2_all[1] - GC2_all[2]) ** 2) / 2
    ...
    def gc3_frame_score(self, seq):
        ...
        return ((GC3_all[0] - GC3_all[1]) ** 2 + (GC3_all[0] - GC3_all[2]) ** 2 + (GC3_all[1] - GC3_all[2]) ** 2) / 2

def extract_gc_features(sequences, max_seq_length):
    gc_coder = GCconder()
    features_list = []
    print(f"Extracting GC features (Total: {len(sequences)})...")
    
    for idx, seq in enumerate(sequences):
        seq = seq.upper().replace('U', 'T')
        
        # 1. Extract 7 scalar features
        f1 = gc_coder.GetGC_Content(seq)
        f2 = gc_coder.GC1(seq)
        f3 = gc_coder.GC2(seq)
        f4 = gc_coder.GC3(seq)
        f5 = gc_coder.gc1_frame_score(seq)
        f6 = gc_coder.gc2_frame_score(seq)
        f7 = gc_coder.gc3_frame_score(seq)
        
        # 2. Convert to numpy array [7]
        feat_vec = np.array([f1, f2, f3, f4, f5, f6, f7], dtype=np.float32)
        
        # 3. Broadcast to sequence length [Max_Seq_Length, 7]
...
def train_one_epoch(dataloader, model, loss_fn, optimizer, device):
    ...
    for features, labels in dataloader:
        ...
        # Calculate loss ONLY for backward pass, do not return/print it
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    print(f"\n===== Fold {fold + 1}/{KFOLD} Training (Feature: GC) =====")
    ...
    for epoch in range(epochs):
        ...
        if val_metrics['ACC'] > best_val_acc:
            ...
            torch.save(model.state_dict(), f"{config.save_model_prefix}{fold + 1}_best_GC.pth")
    ...
    # Test Evaluation
    model.load_state_dict(torch.load(f"{config.save_model_prefix}{fold + 1}_best_GC.pth", map_location=config.device))
...
if __name__ == "__main__":
    ...
    # 2. Extract Features (GC)
    X_cv = extract_gc_features(cv_sequences, MAX_SEQ_LENGTH)
    y_cv = np.array(cv_labels)
    X_test = extract_gc_features(test_sequences, MAX_SEQ_LENGTH)
    ...
    # 4. Summary & Save
    print("\n" + "=" * 100)
    print(f"5-Fold CV Summary (Model: Deep-dsRNAPred | Feature: GC | Dim: {FEATURE_DIM})")
    ...
    pd.DataFrame(all_fold_results).to_excel("Model_Performance_GC.xlsx", index=False)
    print(f"Results saved to Model_Performance_GC.xlsx")