...
SAVE_MODEL_PREFIX = 'stage1model_fold_onehot_'

# Hyperparameters
FEATURE_DIM = 4  # One-Hot encoding (A, C, G, T)
KFOLD = 5
NUM_CLASSES = 2
BATCH_SIZE = 256
EPOCHS = 60
...
# =============================================================================
# 2. ONE-HOT FEATURE EXTRACTION LOGIC
# =============================================================================

class OneHotCoder:
    """
    Generates One-Hot encoding for DNA/RNA sequences.
    Maps A, C, G, T/U to 4-dimensional vectors.
    """
    ...
    def encode(self, seq):
        """
        Encodes a single sequence string into a [Max_Len, 4] matrix.
        """
        # Initialize zero matrix [Max_Len, 4]
        one_hot_matrix = np.zeros((self.max_len, 4), dtype=np.float32)
        ...
        for i in range(seq_len):
            ...
            if base in self.base_map:
                idx = self.base_map[base]
                one_hot_matrix[i, idx] = 1.0
            
        return one_hot_matrix

def extract_onehot_features(sequences, max_seq_length):
    """
    Extracts One-Hot features (4 dim) and handles padding/truncation to [Max_Seq, 4].
    """
    coder = OneHotCoder(max_len=max_seq_length)
    features_list = []
    
    print(f"Extracting One-Hot features (Total: {len(sequences)})...")
    
    for idx, seq in enumerate(sequences):
        # Clean sequence
        seq = seq.upper().replace('U', 'T')
        
        # 1. Encode to [Max_Len, 4] matrix
        onehot_mat = coder.encode(seq)
        
        features_list.append(onehot_mat)
...
def train_one_epoch(dataloader, model, loss_fn, optimizer, device):
    ...
    for features, labels in dataloader:
        ...
        # Calculate loss ONLY for backprop
...
def kfold_train(fold, train_features, train_labels, val_features, val_labels, 
    ...
    print(f"\n===== Fold {fold + 1}/{KFOLD} Training (Feature: One-Hot) =====")
    ...
    for epoch in range(epochs):
        ...
        if val_metrics['ACC'] > best_val_acc:
            ...
            torch.save(model.state_dict(), f"{config.save_model_prefix}{fold + 1}_best_OneHot.pth")
    ...
    # Test Evaluation
    model.load_state_dict(torch.load(f"{config.save_model_prefix}{fold + 1}_best_OneHot.pth", map_location=config.device))
...
if __name__ == "__main__":
    ...
    # 2. Extract Features (One-Hot)
    X_cv = extract_onehot_features(cv_sequences, MAX_SEQ_LENGTH)
    y_cv = np.array(cv_labels)
    X_test = extract_onehot_features(test_sequences, MAX_SEQ_LENGTH)
    ...
    # 4. Summary & Save
    print("\n" + "=" * 100)
    print(f"5-Fold CV Summary (Model: Deep-dsRNAPred | Feature: One-Hot | Dim: {FEATURE_DIM})")
    ...
    pd.DataFrame(all_fold_results).to_excel("Model_Performance_OneHot.xlsx", index=False)
    print(f"Results saved to Model_Performance_OneHot.xlsx")