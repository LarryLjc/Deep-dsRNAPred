...
# =============================================================================
# 2. Optimized Feature Extraction (Replaced with Group Encoding)
# =============================================================================
class GroupFeatureExtractor:
    def __init__(self, max_length=None):
        self.max_length = max_length
        # Encoding dictionary based on user request
        self.group_dict = {
            'A': [1, 1, 1],
            'T': [0, 0, 1],
            'C': [1, 0, 0],
            'G': [0, 1, 0],
            'U': [0, 0, 1], # Handle RNA U as T
            'N': [0, 0, 0]  # Fallback
        }
    ...
    def transform(self, sequences):
        ...
        # Dimensions: Samples x Length x 3 (dictionary value size)
        encoding_array = np.zeros((n_samples, self.max_length, 3), dtype=np.float32)
        ...
        for idx, seq in enumerate(processed_seqs):
            ...
            for i in range(loop_len):
                char = seq[i]
                # Map character to vector, default to [0,0,0] if unknown
                vec = self.group_dict.get(char, [0, 0, 0])
                encoding_array[idx, i, :] = vec
                
        # Flatten for ML models: (n_samples, max_length * 3)
...
def kfold_train_ml(fold, train_seqs, train_labs, val_seqs, val_labs, test_seqs, test_labs):
    ...
    # --- CHANGED: Using the new GroupFeatureExtractor ---
    extractor = GroupFeatureExtractor(max_length=max_len)
    # ----------------------------------------------------
