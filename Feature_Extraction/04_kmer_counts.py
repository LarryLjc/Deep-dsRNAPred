...
OUTPUT_SAVE_PATH = f"Ensemble_Test_Performance_11_ML_Kmer{K_MER_K}.xlsx"
...
# =============================================================================
# 2. Feature Extraction: K-mer Counter
# =============================================================================
class KmerCounter:
    """
...
def generate_feature_matrix(sequences, k=3):
    """
    Generates feature matrix using K-mer extraction (No augmentation).
    """
    extractor = KmerCounter(k=k)
    features = []
    for seq in sequences:
        feat = extractor.extract_features(seq)
...
if __name__ == "__main__":
    if os.path.exists(TRAIN_EXCEL_PATH) and os.path.exists(TEST_EXCEL_PATH):
        ...
        # ---------------------------------------------------------
        # 全局预先提取 K-mer 特征 (无数据泄露风险，全局提取大幅省时)
        # ---------------------------------------------------------
        print(f"Extracting {K_MER_K}-mer Features globally...")
        X_cv_raw = generate_feature_matrix(cv_sequences, k=K_MER_K)
        X_test_raw = generate_feature_matrix(test_sequences, k=K_MER_K)
