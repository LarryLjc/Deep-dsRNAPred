...
# 🌟 更新保存文件名
OUTPUT_SAVE_PATH = "Ensemble_Test_Performance_11_ML_SparseEncoding.xlsx"
...
# =============================================================================
# 2. Optimized Feature Extraction: Sparse Encoding
# =============================================================================
class SparseFeatureExtractor:
    def __init__(self, max_length=None):
        self.max_length = max_length
        # 🌟 修改为 Sparse Encoding 字典 (4位正交向量)
        self.sparse_dict = {
            'A': [1, 0, 0, 0],
            'T': [0, 1, 0, 0],
            'C': [0, 0, 1, 0],
            'G': [0, 0, 0, 1],
            'U': [0, 1, 0, 0], # Handle RNA U as T
            'N': [0, 0, 0, 0]  # Fallback for unknown/padding
        }
    ...
    def transform(self, sequences):
        ...
        for s in sequences:
            # Normalize sequence: Upper case, remove whitespace, U->T
            s = str(s).upper().strip().replace('U', 'T')
            if not s: s = "A" # Handle empty strings safety
        ...
        # 🌟 Dimensions: Samples x Length x 4 (Sparse vector size)
        encoding_array = np.zeros((n_samples, self.max_length, 4), dtype=np.float32)
        
        for idx, seq in enumerate(processed_seqs):
            # Truncate if longer than max_length
            loop_len = min(len(seq), self.max_length)
            
            for i in range(loop_len):
                char = seq[i]
                # Map character to vector, default to [0,0,0,0] if unknown
                vec = self.sparse_dict.get(char, [0, 0, 0, 0])
                encoding_array[idx, i, :] = vec
                
        # Flatten for ML models: (n_samples, max_length * 4)
...
def init_models(random_seed=3407):
    ...
    # 🌟🌟 极速且高准确率的 SVM（应用了之前分析的修复方案）
    base_svm = LinearSVC(
        random_state=random_seed, 
        class_weight='balanced',  # 👈 修复：平衡样本权重，防止预测全为单一类别
        loss='hinge',             # 👈 修复：使用标准的绝对值折叶损失，更适应高维稀疏特征
        max_iter=2000
    )
    # 👈 修复：method='isotonic' 比 sigmoid 更适合大规模稀疏特征的概率校准，不易坍塌
    ...
    if HAS_CUML:
        ...
        # 强制使用 Sklearn 专治 0/1 特征的 BernoulliNB
    ...
    # 3. 三大现代 GBDT 框架
    # 强制 LGBM 和 XGBoost 使用 CPU，避免高维 0/1 稀疏矩阵导致 GPU 分箱报错
...
if __name__ == "__main__":
    if os.path.exists(TRAIN_EXCEL_PATH) and os.path.exists(TEST_EXCEL_PATH):
        ...
        # 🌟 实例化更新后的 SparseFeatureExtractor
        extractor = SparseFeatureExtractor(max_length=max_len)
        
        # ---------------------------------------------------------
        # 全局预先提取 Sparse Encoding 特征 (大幅省时)
        # ---------------------------------------------------------
        print(f"Extracting Sparse Encoding Features globally (Max Length: {max_len})...")
        X_cv_raw = extractor.transform(cv_sequences)
        X_test_raw = extractor.transform(test_sequences)
